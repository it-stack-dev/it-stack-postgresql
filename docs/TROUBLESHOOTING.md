# Troubleshooting — IT-Stack POSTGRESQL
...
4. Verify port is not already in use: ss -tlnp | grep 5432
...
```bash
# Test connectivity
psql -h lab-db1 -U postgresql_user -d postgresql_db -c '\conninfo'
...
1. Check resource usage: docker stats it-stack-postgresql
...
| Log | Path |
|-----|------|
| Application | docker compose logs postgresql |
